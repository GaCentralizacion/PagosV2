...
registrationModule.controller("pagoController", function($scope, $http, $interval, uiGridGroupingConstants, utils, uiGridConstants, $filter, $rootScope, localStorageService, alertFactory, pagoRepository, stats, $window, $routeParams) {
    ...
    ****************************************************************************************************************/
    ...
    $scope.changeFechaAplicacion = function(){
        ...
        today = '01-'+ mm + '-' + yyyy;
        fechaPasada2 = $scope.editar_fecha( today, '-1', 'm','-','/' )

        var f0 = new Date($scope.dtini); // Fecha Elegida
        var f1 = new Date(fechaPasada2); // Periodo Anterior
        var f2 = new Date(); //Hoy
            
        if(f0 < f1){
            alertFactory.warning("La fecha elegida no es permitida, elija unicamente fechas del mes anterior");
            result = false;
        }
        else if(f0 > f2){
            alertFactory.warning("La fecha elegida no es permitida, no se puede trabajar con fechas futuras");
            result = false;
        }
        else{
            result = true;

            var dd1 = String(f0.getDate()).padStart(2, '0');
            var mm1 = String(f0.getMonth() + 1).padStart(2, '0'); //January is 0!
            var yyyy1 = f0.getFullYear();

            $scope.fechaAplicacion = yyyy1 + '/' + mm1 + '/' + dd1;
        }
    ...
    ****************************************************************************************************************/
    ...
    $scope.BuscarTesoreria = function() {
        ...
        $rootScope.gridtesoreriaoptions.columnDefs = [
            { name: 'idLotePago', displayName: 'Lote', width: '5%', enableCellEdit: false, visible: true },
            { name: 'fecha', displayName: 'Fecha Lote', type: 'date', cellFilter: 'date:"dd/MM/yyyy"', width: '10%', enableCellEdit: false },
            ...
            { name: 'generaTXT', displayName: 'TXT', width: '5%', cellTemplate: '<div ng-show="(row.entity.estatus==3)||(row.entity.estatus==4 &&(row.entity.bancoPagador.substring(0,7)!=\'BANAMEX\'))"><button class="btn btn-default" ><span class="glyphicon glyphicon-list-alt"  ng-click="grid.appScope.GenerarArchivoBtn(row.entity.idEmpresa, row.entity.idLotePago, row.entity.bancoPagador.substring(0,7))")" ></span></button></div>' },
            { name: 'buscaErrores', displayName: 'Verificar', width: '15%',  cellTemplate: '<div ng-show="(row.entity.bancoPagador.substring(0,7)==\'BANAMEX\')"><button class="btn btn-default" ><span class="glyphicon glyphicon-list-alt"  ng-click="grid.appScope.VerificarErroresBtn(row.entity.idLotePago)"></span></button></div>' },
    ...
    $scope.BuscarLotesxFecha = function(fechaini, fechafin) {
     ...
     $scope.BuscarLotesxFechaTesoreria = function(fechaini, fechafin) {
        ...
        .then(function successCallback(response) {
            $rootScope.gridtesoreriaoptions.data = response.data;
    ...
    var ConfiguraGrid = function() {
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [{
                ...
                ,
    { name: 'saldo', displayName: 'Saldo', width: '15%', cellFilter: 'currency', enableCellEdit: false , treeAggregationType: uiGridGroupingConstants.aggregation.SUM, customTreeAggregationFinalizerFn: function( aggregation ) {
        aggregation.rendered = aggregation.value
                ...
                { name: 'documento', displayName: '# Documento', width: '15%', enableCellEdit: false, headerTooltip: 'Documento # de factura del provedor', cellClass: 'cellToolTip',cellTemplate: '<div style="text-align: center;"><span align="center"><a class="urlTabla" href ng-click="grid.appScope.VerDocumento(row.entity)">{{row.entity.documento}}</a></span></div>' },
                {
                    name: 'agrupar', 
      field: 'agrupar',
      displayName: 'No agrupar',width: '8%',
      type: 'boolean',
    ...
    ************************************************************************************************************/
    $scope.GenerarArchivoBtn = function (varidempresa, varidlote,  varNombreBanco){
        ...
        $rootScope.varNombreBanco = varNombreBanco;  
       
        if (varNombreBanco == 'BANAMEX'){
            $scope.mostrarDetallePagosBanamex(varidlote);
        }else{
        $('#modalTXT').modal('show');
        }                                         
        ...
        pagoRepository.getlogPagosBanamex($rootScope.varidlote).then(function(res){
             if(res.data.length > 0){
                ...
                $('#modalLogBanamex').modal('show');
                 ...
                 alertFactory.infoTopFull("En espera de respuesta del banco");
             }
        });
    ...
    $scope.generarPdf = function(){
        ...
        doc.text('LOTE DE PAGO ' + $rootScope.varidlote, 180,65);
        doc.setFontSize(10);
        doc.text(('0' + date.getDate()).slice(-2)+ '/' + ('0' + (date.getMonth() + 1)).slice(-2) + '/' +  date.getFullYear() , 380,75);
        doc.addImage(logo,'PNG', 30, 40, 98,30);
        doc.addImage(img,'PNG',10, 100, 440, 270);
        ...
        pagoRepository.getDetalleBitacoraPagos($rootScope.varidlote, $rootScope.Referencia, $rootScope.tipo).then(function(res){
            if(res.data.length > 0){
                ...
                $('#modalDocumentosRechazados').modal('show');
                 ...
                 alertFactory.infoTopFull("Sin datos que mostrar");
             }
        });
    ...
    $scope.GenerarArchivo = function(varidempresa, varidlote,dtfechaini,dtfechafin,varNombreBanco) {
        ...
        if (varNombreBanco == 'BANAMEX'){
            ...
            if (confirm('¿Esta seguro de que desea enviar los pagos a banamex?')){ 
        ...
        pagoRepository.setArchivo(varidempresa, varidlote)
            .then(function successCallback(response) {
                ...
                setTimeout(function() {
                    ...
                    alertFactory.infoTopFull('Se enviaron los documentos de Banamex');
                       
            
                                                                                      
                    
                    $scope.FromDate = date.getFullYear() + '-' + ('0' + (date.getMonth() + 1)).slice(-2) + '-' + ('0' + date.getDate()).slice(-2);
                    $scope.BuscarLotesxFechaTesoreria($scope.FromDate,$scope.FromDate);
                    //$scope.buscarLotes();
                     //$scope.BuscarTesoreria();
